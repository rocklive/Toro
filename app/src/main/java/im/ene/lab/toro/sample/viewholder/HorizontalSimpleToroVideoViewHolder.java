 ...
 */
public class HorizontalSimpleToroVideoViewHolder extends ToroVideoViewHolder {

  private final String TAG = getClass().getSimpleName();

  public static final int LAYOUT_RES = R.layout.vh_toro_video_simple_horizontal;

  private ImageView mThumbnail;
  private TextView mInfo;

  public HorizontalSimpleToroVideoViewHolder(View itemView) {
    super(itemView);
  ...
  @Override public void onPlaybackStarted() {
    mThumbnail.animate().alpha(0.f).setDuration(250).setListener(new AnimatorListenerAdapter() {
      @Override public void onAnimationEnd(Animator animation) {
        HorizontalSimpleToroVideoViewHolder.super.onPlaybackStarted();
      }
    }).start();
  ...
  @Override public void onPlaybackPaused() {
    mThumbnail.animate().alpha(1.f).setDuration(250).setListener(new AnimatorListenerAdapter() {
      @Override public void onAnimationEnd(Animator animation) {
        HorizontalSimpleToroVideoViewHolder.super.onPlaybackPaused();
      }
    }).start();
